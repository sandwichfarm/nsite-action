...
A GitHub Action to deploy static website files to Blossom/Nostr using the [nsyte CLI](https://github.com/sandwichfarm/nsyte).
...
*   Masks sensitive secrets (`nbunksec`, `relays`, `servers`) in workflow logs.
    ...
    steps:
      ...
      - name: Deploy with nsite-action
        ...
        with:
          ...
          # Required: Nostr relays to publish to (YAML array)
          relays: |
            wss://relay.damus.io
            wss://relay.snort.social

          # Required: Blossom servers to upload to (YAML array)
          servers: |
            wss://blossom.yourserver.com
...
| --------------- | ------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| `nsyte_version` | The version tag of `nsyte` to use (e.g., "v0.5.0"). Must exist in `sandwichfarm/nsyte` releases.             | `false`  | `latest` |
...
| `relays`        | Array of Nostr relay WebSocket URIs to publish to (YAML array format).                                        | `true`   |          |
| `servers`       | Array of Blossom server WebSocket URIs to upload to (YAML array format).                                      | `true`   |          |
...
*   **Relay/Server URIs**: While not cryptographic secrets, treat your `relays` and `servers` lists with care, especially if they point to private or restricted endpoints. These are logged by the action but masked in the command execution output.
...
1.  Run `make test-local` to execute a basic script that simulates platform detection and shows example command construction.
2.  For full action testing, you might need tools like `act` (though composite actions can have limitations with `act`).
3.  Push changes to a branch and observe the test workflow run in GitHub Actions.

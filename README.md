...
3. **Add to workflow**:
    ...
    - name: Deploy to Nostr/Blossom
      ...
      with:
        ...
        version: 'v0.27.0'
        title: 'My Site'
        description: 'A static site published with NIP-5A metadata'
        relays: |
          wss://relay.nsite.lol
        servers: |
   ...
   To use a config generated by `nsyte init`, commit the JSON config and pass
   its path relative to the repository root. The action validates the config
   before deployment:
   ```yaml
...
| `title` | No | `''` | Human-readable NIP-5A site title written into the effective nsyte config |
| `description` | No | `''` | Short NIP-5A site description written into the effective nsyte config |
| `config_path` | No | `''` | Path to an nsyte config JSON file relative to the repository root. Useful with configs generated by `nsyte init` |
...
- Publishes optional NIP-5A `title` and `description` metadata through nsyte config
- Can deploy from a committed nsyte config via `config_path`
